...
from tm.commands._shared import DbPathOption
...
def set_key(db_path: DbPathOption = None) -> None:
    ...
    if not state.sqlcipher_available:
        typer.echo(
            "error: this sqlite3 driver does not expose SQLCipher; not storing key",
            err=True,
        )
        raise typer.Exit(1)
    if resolved_db.exists() and resolved_db.stat().st_size > 0:
        typer.echo(
            "error: refusing to store a new key for an existing database; "
    ...
    if not key:
        typer.echo("error: key must not be empty", err=True)
        raise typer.Exit(1)
    try:
